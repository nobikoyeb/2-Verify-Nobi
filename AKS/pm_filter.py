...
import pyrogram
from info import MAX_BTN, TUTORIAL_2, BIN_CHANNEL, USERNAME, ADMINS, LANGUAGES, AUTH_CHANNEL, SUPPORT_GROUP, IMDB, IMDB_TEMPLATE, LOG_CHANNEL, LOG_VR_CHANNEL, TUTORIAL, FILE_CAPTION, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, IS_PM_SEARCH, QR_CODE, DELETE_TIME, TWO_VERIFY_GAP, IS_VERIFY
...
from utils import temp, get_settings, is_check_admin, get_status, get_hash, get_name, get_size, save_group_settings, is_req_subscribed, get_poster, get_status, get_readable_time, get_shortlink, replace_words, get_name
from database.users_chats_db import db
from database.aks_files import Media, get_search_results, get_bad_files, get_file_details
...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"📂{get_size(file.file_size)}≽ {file.file_name}", callback_data=f'file#{file.file_id}')]
                for file in files
              ]
    btn.insert(0,[
        InlineKeyboardButton("💸 ʙᴜʏ 💸", url=f"https://t.me/{temp.U_NAME}?start=buy_premium"),
        InlineKeyboardButton("🈲️ ʟᴀɴɢᴜᴀɢᴇs 🈲️", callback_data=f"languages#{key}#{req}#{offset}")
    ])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        bot_id = client.me.id
        stream = await db.get_stream_url(bot_id)
        file_id = query.data.split('#', 1)[1]
        AKS = await client.send_cached_media(
            chat_id=BIN_CHANNEL,
            file_id=file_id)
        online = f"{stream}/watch/{str(AKS.id)}/{quote_plus(get_name(AKS))}?hash={get_hash(AKS)}"
        download = f"{stream}/{str(AKS.id)}/{quote_plus(get_name(AKS))}?hash={get_hash(AKS)}"
        btn= [[
        ...
        buttons = [[
            InlineKeyboardButton('♻️ ᴄᴜꜱᴛᴏᴍɪᴢᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ ♻️', callback_data='customize')
        ],[
            InlineKeyboardButton('⋞ ʙᴀᴄᴋ', callback_data='start'),
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                return await aks_spelling_check(msg)
            return
    else:
    ...
    temp.CHAT[message.from_user.id] = grp_id
    ...
    if offset != "":
        if total_results >= 3:
            btn.insert(0,[
                InlineKeyboardButton("💸 ʙᴜʏ 💸", url=f"https://t.me/{temp.U_NAME}?start=buy_premium"),
                InlineKeyboardButton("🈲️ ʟᴀɴɢᴜᴀɢᴇs 🈲️", callback_data=f"languages#{key}#{req}#0")
            ])
        else:
            btn.insert(0,[
                InlineKeyboardButton("💸 ʙᴜʏ 💸", url=f"https://t.me/{temp.U_NAME}?start=buy_premium")
    ...
    if offset != "":
        ...
        if not settings["link"]:
            btn.append(
                [InlineKeyboardButton(text="🚫 ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇꜱ 🚫", callback_data="buttons")]
            )
    ...
    if imdb and imdb.get('poster'):
        ...
        k=await message.reply_text(cap + links + del_msg, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
        if settings['auto_delete']:
            await asyncio.sleep(DELETE_TIME)
            await k.delete()
            try:
            ...
            await message.reply_text(cap + links + del_msg, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)           

async def aks_spelling_check(message):
    mv_id = message.id
    search = message.text
    chat_id = message.chat.id
    settings = await get_settings(chat_id)
